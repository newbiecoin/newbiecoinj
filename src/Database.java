...
public class Database {
	...
	public void createTables() {
		try {
			...
			// Bets
			executeUpdate("CREATE TABLE IF NOT EXISTS bets(tx_index INTEGER PRIMARY KEY, tx_hash TEXT UNIQUE, block_index INTEGER, source TEXT, bet INTEGER, bet_bs INTEGER,  profit INTEGER, nbc_supply INTEGER, rolla REAL, rollb REAL, roll REAL, resolved TEXT, validity TEXT)");
			executeUpdate("CREATE INDEX IF NOT EXISTS block_index_idx ON bets (block_index)");

			// Burns
			executeUpdate("CREATE TABLE IF NOT EXISTS burns(tx_index INTEGER PRIMARY KEY, tx_hash TEXT UNIQUE, block_index INTEGER, source TEXT, burned INTEGER, earned INTEGER, validity TEXT,destination TEXT)");
			executeUpdate("CREATE INDEX IF NOT EXISTS validity_idx ON burns (validity)");
			executeUpdate("CREATE INDEX IF NOT EXISTS source_idx ON burns (source)");
            executeUpdate("CREATE INDEX IF NOT EXISTS dest_idx ON burns (destination)");
